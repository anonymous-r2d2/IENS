...
class Analogy(Model):
    ...
    def loss_def(self):
        ...
        loss_func = tf.reduce_sum(tf.nn.softplus(- y * res), 0, keep_dims=False)
        regul_func = tf.reduce_sum(e1_h ** 2) + tf.reduce_sum(e1_t ** 2) + tf.reduce_sum(e2_h ** 2) + tf.reduce_sum(
            e2_t ** 2) + tf.reduce_sum(r1 ** 2) + tf.reduce_sum(r2 ** 2) + tf.reduce_sum(e_h ** 2) + tf.reduce_sum(
            e_t ** 2) + tf.reduce_sum(rel ** 2)
